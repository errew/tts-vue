    ...
    <el-form :model="formConfig" label-width="120px" label-position="top">
        ...
        >
          ...
          >
            <div style="display: flex; justify-content: space-between">
              <span style="margin-right: 5px">{{ item.DisplayName + '-' + item.LocalName }}</span>
              <el-button size="small" type="success" circle @click.stop="audition(item.ShortName)"
      ...
      <el-form-item :label="t('options.rolePlaying')">
        ...
        >
          ...
          >
            <div style="display: flex; justify-content: start">
              <span style="margin-right: 5px">{{ getRoleDes(item)?.emoji }}</span>
      ...
      <el-form-item class="startBtn">
        <div class="configOption">
          <el-button color="#626aef" :dark="false" plain size="small" @click="saveConfig">{{
            t('options.saveConfig')
          }}</el-button>
          <el-select-v2
            class="get-cfg"
...
const { inputs, formConfig, page, tableData, currConfigName, config, isLoading } =
...
const saveConfig = () => {
  ElMessageBox.prompt(t('messages.saveConfigPrompt'), t('messages.saveConfig'), {
...
const voiceSelectList = ref(oc.findVoicesByLocaleName(formConfig.value.languageSelect))
...
const voiceSelectChange = (value: string) => {
  ...
  formConfig.value.voiceStyleSelect =
    voiceStyleSelectList.value.length > 0 ? voiceStyleSelectList.value[0] : ''
